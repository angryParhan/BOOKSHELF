...
const user = {
  async addBook (req, res) {
    if (!req.body.book_id || (!req.body.uid && !req.body.library_id)) {
      return errorHandler(res, 'Bad request!')
    } else if (!req.body.library_id) {
      req.body.library_id = req.body.uid + '-fav'
    }
    if ((await connection.query(`SELECT * FROM libraries WHERE library_id = '${req.body.library_id}'`)).rows.length) {
      return errorHandler(res, 'Bad request!')
  ...
  async addLibrary (req, res) {
    if (!req.body.uid || req.body.library_id) {
      return errorHandler(res, 'Bad request!')
    }
    ...
    connection
      .query(`INSERT INTO bookshelf.user_library (user_id, library_id) VALUES ('${req.body.uid}', '${req.body.library_id}');`)
      .then(() => res.send({ success: true }))
      .catch(e => errorHandler(res, e))
  },
}
